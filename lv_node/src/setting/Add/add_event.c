...
// 点击键盘页面的添加按钮事件
static void AddDeviceNameEvent(lv_event_t *e)
{
    lv_event_code_t code = lv_event_get_code(e);
    lv_obj_t *obj = lv_event_get_target(e);
    ...
    if (code == LV_EVENT_CLICKED)
    {
        ...
        lv_label_set_text(DeviceTextareaLabel, inputVal);                                             // 设置标签的值
...
static void DeviceTextareaEvent(lv_event_t *e)
{
    ...
    {
        printf("点击设备名称\n");
        lv_keypage_create(lv_scr_act(),AddDeviceNameEvent);
    }
}
...
lv_obj_t *lv_device_textarea(lv_obj_t *parent, uint32_t c, lv_coord_t x_ofs, lv_coord_t y_ofs, lv_event_cb_t event_cb)
{
    ...
    DeviceTextareaLabel = lv_label_create(DeviceTextarea); // 创建标签
