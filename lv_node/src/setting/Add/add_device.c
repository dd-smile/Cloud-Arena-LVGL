...
static void add_init_data(const DeviceInfo *deviceInfo)
{ 

    if (deviceInfo->index > 0 && deviceInfo->index <= MAX_DEVICES)
    {
        addNode(&head, deviceInfo->index, deviceInfo->addr, deviceInfo->type,deviceInfo->name);

        lv_device_list_t *node = searchNode(head, deviceInfo->index); // 查找设备

        // create_device(node, deviceInfo->type); // 创建设备

        // saveLinkedListToJson(head, "devices.json"); // 保存设备信息到json文件
        // updateJsonFile(head, "devices.json");       // 更新json文件
    }
    else
    {
        // SHOW_ERROR_POPUP(ui_main_page.setting, "错误提示", "确定", "取消", "设备位置已满，无法添加新设备", lv_back);
    }
}

static void confirm_add_device_event_cb(lv_event_t *e)
{
...
static lv_obj_t *CreateDevicePopupBox(lv_obj_t *parent)
{
    ...
    lv_obj_clear_flag(iamge, LV_OBJ_FLAG_SCROLLABLE);
    ...
    lv_obj_add_event_cb(confirm_btn, confirm_add_device_event_cb, LV_EVENT_CLICKED, NULL); // 添加返回事件
