...
static lv_obj_t *CreateLanguageBox(lv_obj_t *parent)
{
  ...
  const char *options[] = {"中文(CN)", "英文(US)"}; // 选项文本列表
  int16_t y = -18;
...
static lv_obj_t *CreatePopupBox(lv_obj_t *parent)
{
  ...
  lv_obj_t *confirm_btn = btn_create_text(iamge, false, "确定", 135, 260);  // 创建按钮
  lv_obj_add_event_cb(confirm_btn, lv_back_Mask, LV_EVENT_CLICKED, NULL); // 添加返回事件
  lv_obj_t *Back_btn = btn_create_text(iamge, false, "取消", 280, 260); // 创建按钮
...
void CreateLanguage(lv_obj_t *parent)
{
  ...
  card_create_24_text(parent, "语言设置", 10, -15); // 创建标题
  card_create_16_text(parent, "选择语言", -40, 35);      // 创建标题
