...
void Controls_lv_event_cb(lv_event_t *e)
{
...
static void create_button(lv_obj_t *obj, const char *text, lv_coord_t x_ofs, uint8_t index)
{
    ...
    lv_obj_add_event_cb(btn, Controls_lv_event_cb, LV_EVENT_ALL, index);
...
static lv_obj_t *lv_box_create(lv_obj_t *parent, int x, int y, const char *text, const char *text1, const char *text2)
{
    ...
    create_button(btn_card, text, -60, 0);
    create_button(btn_card, text1, 0, 2);
    create_button(btn_card, text2, 60, 1);
