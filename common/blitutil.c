...
void SstarBlitNormal(Surface *pSrcSurface, Surface *pDstSurface, RECT *pRect)
{
    ...
    stDstRect.s32Xpos = 0;
    stDstRect.s32Ypos = 0;
    stDstRect.u32Width = pDstSurface->w;
    stDstRect.u32Height = pDstSurface->h;
    ...
    //start = clock();
    printf("%s %d: s.c,s.w,s.h,s.s,sr.x,sr.y.sr.w,sr.h,d.c,d.w,d.h,d.s,dr.x,dr.y,dr.w,dr.h[%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d] \n",__FUNCTION__, __LINE__,
        stSrc.eColorFmt, stSrc.u32Width, stSrc.u32Height, stSrc.u32Stride, stSrcRect.s32Xpos, stSrcRect.s32Ypos, stSrcRect.u32Width, stSrcRect.u32Height,
        stDst.eColorFmt, stDst.u32Width, stDst.u32Height, stDst.u32Stride, stDstRect.s32Xpos, stDstRect.s32Ypos, stDstRect.u32Width, stDstRect.u32Height);
    u32Ret = MI_GFX_BitBlit(&stSrc, &stSrcRect, &stDst, &stDstRect, &stOpt, &u16Fence);
    printf("u32Ret=0x%d",u32Ret);
    MI_GFX_WaitAllDone(FALSE, u16Fence);
}